...
function bindTabs() {
    ...
    tabs.forEach(tab => {
        tab.addEventListener('click', () => {
            if (tab.dataset.tab === 'communities') {
...
async function renderPeople(query, filters, sortValue) {
    ...
    return results.map(m => {
        ...
        return `
        <div class="match-card" id="user-card-${m.profile.id}" style="display:flex; flex-direction:column; justify-content:space-between;">
            <div class="match-header">
                ${avatarHtml}
                <div class="match-info">
                    <h3>${m.profile.full_name || 'Unknown'}</h3>
                    <p>@${m.profile.username} • ${m.profile.country || 'Global'}</p>
                </div>
...
async function renderSkills(query, filters, sortValue) {
    ...
    return results.map(s => `
        <div class="auth-card" style="max-width: 100%; padding: 1.5rem; display:flex; flex-direction:column; justify-content:space-between;">
            <div>
            ...
            <div style="margin-top: 1.5rem;">
                ...
                <div style="display:grid; grid-template-columns:1fr; gap:0.5rem;">
                    <button class="btn btn-primary" data-explore-skill="${escapeHTML(s.name)}" data-explore-cat="${escapeHTML(s.cat || 'General')}" onclick="window.exploreSkill(this.dataset.exploreSkill, this.dataset.exploreCat)"><i class="fas fa-magic"></i> AI Insights</button>
                </div>
            </div>
        </div>
    `).join('');
...
async function renderCommunities(query, sortValue) {
    ...
    return comms.map(c => `
        <div class="auth-card" style="max-width: 100%; padding: 1.5rem; display: flex; flex-direction: column; justify-content: space-between; height: 100%;">
            <div>
...
window.exploreSkill = async (skillName, category) => {
    ...
    if (content) {
        content.innerHTML = `<div class="loader" style="margin: 2rem auto;"></div><p style="text-align:center; color:var(--color-text-muted);">Exploring skills...</p>`;
        modal.classList.remove('hidden');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (reviewId) {
        setTimeout(() => {
            const card = document.getElementById('user-card-' + reviewId);
            if (card) {
                card.scrollIntoView({ behavior: 'smooth', block: 'center' });
                card.style.boxShadow = '0 0 15px var(--color-primary)';
